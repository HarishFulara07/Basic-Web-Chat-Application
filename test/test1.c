/*
* Test program to check that atmost 20 users can simultaneously register at a time
*/
...
void * register_user(void * arg) {
	...
	if(read_msg > 0) {
		...
		fprintf(stderr, "ERROR in receiving registration ACK from the server.\n\n");
		exit(1);
	}
...
int main() {
	...
	for (i = 0; i < 21; ++i) {
		pthread_create(&p[i], NULL, register_user, &i);
	}
	...
	printf("The 21st client is unable to register because the server can only handle 20 registration connections at a time.\n");	
